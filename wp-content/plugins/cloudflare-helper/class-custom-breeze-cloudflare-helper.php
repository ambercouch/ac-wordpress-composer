 ...
 * @final
 */
final class Custom_Breeze_CloudFlare_Helper {
	 ...
	 * @static
	 */
	public static function purge_cloudflare_cache_urls( array $specific_urls = array() ) {
		...
		$specific_urls = ( new Custom_Breeze_CloudFlare_Helper )->remove_not_url_elements( $specific_urls );
		if ( true === self::is_log_enabled() ) {
			error_log( 'single url : ' . var_export( $specific_urls, true ) );
		}

		// Call cache reset.
		return ( new Custom_Breeze_CloudFlare_Helper )->request_cache_reset( $specific_urls, 'purge-fpc-url' );
	}
	...
	public static function reset_all_cache( array $home_url = array() ) {
		...
		if ( is_multisite() ) {
			if ( is_subdomain_install() ) {
				$home_url = ( new Custom_Breeze_CloudFlare_Helper )->clear_domain_purge_urls( $home_url );
				if ( true === self::is_log_enabled() ) {
		...
		return ( new Custom_Breeze_CloudFlare_Helper )->request_cache_reset( $home_url, $purge_request_endpoint );
	}
